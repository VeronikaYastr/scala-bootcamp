package com.evolutiongaming.bootcamp.homework.variance
...
object Hometask extends App {
  ...
  object Matrix {
    ...
    implicit val comonad: Comonad[Matrix] = new Comonad[Matrix] {
      override def extract[A](x: Matrix[A]): A = x.matrix.head.head

      override def coflatMap[A, B](fa: Matrix[A])(f: Matrix[A] => B): Matrix[B] = {
        val res = for {
          x <- fa.matrix.indices
          y <- fa.matrix(0).indices
        } yield f(new Matrix(fa.matrix, Focus.of(x, y).getOrElse(Focus.Zero)) {})
        Matrix.of(res, fa.matrix.size).getOrElse(new Matrix(Vector.empty, Focus.Zero) {})
      }

      override def map[A, B](fa: Matrix[A])(f: A => B): Matrix[B] = new Matrix(fa.matrix.map(x => x.map(f)), fa.focus) {}
    }
  }
